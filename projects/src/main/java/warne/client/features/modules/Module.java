package warne.client.features.modules;
...
import warne.client.core.manager.client.CommandManager;
...
import warne.client.features.modules.client.Windows;
import warne.client.features.modules.client.UnHook;
...
import static warne.client.WarneClient.LOGGER;
import static warne.client.core.Managers.NOTIFICATION;
...
public abstract class Module {
    ...
    protected void sendPacketSilent(Packet<?> packet) {
        if (mc.getNetworkHandler() == null) return;
        WarneClient.core.silentPackets.add(packet);
        mc.getNetworkHandler().sendPacket(packet);
    }
    ...
    public void enable() {
        ...
        if (isOn()) WarneClient.EVENT_BUS.subscribe(this);
        if (fullNullCheck()) return;

        LogUtils.getLogger().info("[Warne Client] enabled " + this.getName());
    ...
    public void disable() {
        try {
            WarneClient.EVENT_BUS.unsubscribe(this);
        ...
        LOGGER.info("[Warne Client] disabled {}", getName());
