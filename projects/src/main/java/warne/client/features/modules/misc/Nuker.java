package warne.client.features.modules.misc;
...
import warne.client.events.impl.EventAttackBlock;
import warne.client.events.impl.EventSetBlockState;
...
import warne.client.features.modules.player.SpeedMine;
...
public class Nuker extends Module {
    ...
    private final Setting<Boolean> areaNuke = new Setting<>("AreaNuke", true);
    private final Setting<Integer> areaSize = new Setting<>("AreaSize", 1, 1, 4, v -> areaNuke.getValue());

    private Block targetBlockType;
    private BlockData blockData;
    private final java.util.List<BlockPos> areaTargets = new ArrayList<>();
    ...
    public void onEnable() {
        ...
        nukerThread.setName("WarneClient-NukerThread");
    ...
    public synchronized void breakBlock() {
        ...
        if (areaNuke.getValue())
            breakAreaBlocks(blockData.bp);
    }

    /**
     * Breaks every valid block in a cube around the current target.
     * AreaSize = 1 -> 3x3x3 (the classic "3x3"), AreaSize = 2 -> 5x5x5, etc.
     */
    private void breakAreaBlocks(BlockPos center) {
        areaTargets.clear();
        int radius = areaSize.getValue();
        ...
        for (int dx = -radius; dx <= radius; dx++) {
            for (int dy = -radius; dy <= radius; dy++) {
                for (int dz = -radius; dz <= radius; dz++) {
                    ...
                    if (flatten.getValue() && pos.getY() < mc.player.getY()) continue;
                    if (avoidLava.getValue() && checkLava(pos)) continue;
                    if (PlayerUtility.squaredDistanceFromEyes(pos.toCenterPos()) > range.getPow2Value()) continue;
                    ...
                    try {
                        sendSequencedPacket(id -> new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, pos, Direction.UP, id));
        ...
        if (renderBp != null) {
            ...
            if (areaNuke.getValue()) {
                for (BlockPos pos : areaTargets) {
                    Render3DEngine.drawBoxOutline(new Box(pos), color1, 1);
                    Render3DEngine.drawFilledBox(stack, new Box(pos), Render2DEngine.injectAlpha(color1, 60));
                }
            }
        }
