package warne.client.utility.world;
...
import warne.client.injection.accesors.IExplosion;
...
public final class ExplosionUtility {
    ...
    public static int getProtectionAmount(ItemStack stack) {
        var registry = mc.world.getRegistryManager().get(Enchantments.BLAST_PROTECTION.getRegistryRef());
        var blastEntry = registry.getEntry(Enchantments.BLAST_PROTECTION);
        var protEntry = registry.getEntry(Enchantments.PROTECTION);
        int modifierBlast = blastEntry.isPresent() ? EnchantmentHelper.getLevel(blastEntry.get(), stack) : 0;
        int modifier = protEntry.isPresent() ? EnchantmentHelper.getLevel(protEntry.get(), stack) : 0;
