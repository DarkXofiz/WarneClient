...
import static warne.client.core.manager.client.ConfigManager.IMAGES_FOLDER;
...
public final class ThunderUtility {
    ...
    public static @NotNull String getAuthors() {
        List<String> names = WarneClient.MOD_META.getAuthors()
                .stream()
                .map(Person::getName)
                .toList();
    ...
    public static void syncVersion() {
        try {
            if (!new BufferedReader(new InputStreamReader(new URL("https://raw.githubusercontent.com/Yukile394/WarneClient/main/version.txt").openStream())).readLine().equals(WarneClient.VERSION))
                WarneClient.isOutdated = true;
    ...
    public static void parseStarGazer() {
        ...
        try {
            for (int page = 1; page <= 3; page++) {
                URL url = new URL("https://api.github.com/repos/Yukile394/WarneClient/stargazers?per_page=100&page=" + page);
    ...
    public static void syncContributors() {
        try {
            URL list = new URL("https://raw.githubusercontent.com/Yukile394/WarneClient/main/team.txt");
            BufferedReader in = new BufferedReader(new InputStreamReader(list.openStream(), StandardCharsets.UTF_8));
            String inputLine;
            int i = 0;
            while ((inputLine = in.readLine()) != null) {
                WarneClient.contributors[i] = inputLine.trim();
                i++;
            }
            in.close();
        } catch (Exception e) {
    ...
    public static void parseCommits() {
        try {
            URL url = new URL("https://api.github.com/repos/Pan4ur/WarneClient-Recode/commits?per_page=50");
            BufferedReader in = new BufferedReader(new InputStreamReader(url.openStream(), StandardCharsets.UTF_8));

            changeLog.add("Changelog [Recode; Date: " + WarneClient.BUILD_DATE + "; GitHash:" + WarneClient.GITHUB_HASH + "]");
            changeLog.add("\n");
