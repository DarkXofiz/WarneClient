package warne.client.core.manager.player;
...
import warne.client.events.impl.EventPostTick;
import warne.client.events.impl.PacketEvent;
import warne.client.events.impl.TotemPopEvent;
import warne.client.features.modules.Module;
import warne.client.features.modules.combat.AntiBot;
    ...
    public void onPacketReceive(PacketEvent.Receive event) {
        ...
        if (event.getPacket() instanceof EntityStatusS2CPacket pac) {
            if (pac.getStatus() == EntityStatuses.USE_TOTEM_OF_UNDYING) {
                ...
                WarneClient.EVENT_BUS.post(new TotemPopEvent((PlayerEntity) ent, popList.get(ent.getName().getString())));
            }
        }
    }
