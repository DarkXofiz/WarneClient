...
import warne.client.features.modules.client.Notifications;
...
public class NotificationManager implements IManager {
        ...
        if (SystemTray.isSupported()) {
            try {
                if (trayIcon == null) {
                    ...
                    trayIcon = new TrayIcon(image, "Warne Client");
                    trayIcon.setImageAutoSize(true);
                    trayIcon.setToolTip("Warne Client");
                    tray.add(trayIcon);
                }
    ...
    private void mac(final String message) {
        ...
        processBuilder.command("osascript", "-e", "display notification \"" + message + "\" with title \"WarneClient\"");
        try {
    ...
    private void linux(final String message) {
        final ProcessBuilder processBuilder = new ProcessBuilder();
        processBuilder.command("notify-send", "-a", "Warne Client", message);
