package warne.client.features.hud.impl;
...
import warne.client.features.modules.combat.AutoAnchor;
import warne.client.features.modules.combat.AutoCrystal;
import warne.client.features.modules.misc.NameProtect;
import warne.client.setting.Setting;
import warne.client.setting.impl.ColorSetting;
import warne.client.utility.ThunderUtility;
...
import warne.client.utility.render.animation.EaseOutCirc;
...
public class TargetHud extends HudElement {
    ...
    private final Setting<ModeEn> Mode = new Setting<>("Mode", ModeEn.WarneClient);
    ...
    public void onEnable() {
        try {
            ...
            sendMessage(".minecraft -> WarneClientRecode -> misc -> images -> thud.png");
        }
    }
    ...
    public void onRender2D(DrawContext context) {
        ...
        if (animation.getAnimationd() > 0) {
            ...
            switch (Mode.getValue()) {
                case WarneClient -> renderWarneClient(context, health, animationFactor);
    ...
    private void renderNurik(DrawContext context, float health, float animationFactor) {
        ...
        if (target instanceof PlayerEntity) {
            ...
            // Elytra Target Indicator
            drawElytraIndicator(context, (PlayerEntity) target);
        }
    }
    ...
    private void renderWarneClient(DrawContext context, float health, float animationFactor) {
        float hurtPercent = target.hurtTime / 6f;
    ...
    private void drawElytraIndicator(DrawContext context, PlayerEntity targetPlayer) {
        ItemStack chestStack = targetPlayer.getEquippedStack(EquipmentSlot.CHEST);
        boolean hasElytra = chestStack.getItem() instanceof ElytraItem;
        boolean isGliding = targetPlayer.isFallFlying();

        if (!hasElytra && !isGliding) return;

        // Elytra durumu rengi: uçuyor → mavi, sadece sahip → gri
        Color elytraColor = isGliding ? new Color(0x5BC8FF) : new Color(0x8D8D8D);

        String elytraText = isGliding ? "✈ ELYTRA [AKTIF]" : "✈ ELYTRA";

        // Durability hesapla
        if (hasElytra && chestStack.isDamageable()) {
            int dur = chestStack.getMaxDamage() - chestStack.getDamage();
            int maxDur = chestStack.getMaxDamage();
            int durPercent = (int) ((float) dur / maxDur * 100);
            elytraText += " " + durPercent + "%";
            if (durPercent < 20) elytraColor = new Color(0xFF4444);
            else if (durPercent < 50) elytraColor = new Color(0xFFAA00);
        }

        FontRenderers.settings.drawString(context.getMatrices(), elytraText, getPosX() + 55, getPosY() + 25, elytraColor.getRGB());
    ...
    public enum ModeEn {
        WarneClient, NurikZapen, CelkaPasta
    }
}