...
public final class ElytraAura extends Module {
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       ROTATION + ORBIT
       escapeMode: dist < radius*1.1 → hedeften kaç, zorla yukarı bak.
       Normal mod: orbit + proximity fix'ler.
    ═══════════════════════════════════════════════════════════════════════ */
    ...
    private void followAndOrbit(Entity target) {
        ...
        boolean escapeMode = dist < radius * 1.1;

        Vec3d predicted = tPos;
        if (!escapeMode && interceptTarget.getValue()
        ...
        if (escapeMode) {
            double dx0  = mc.player.getX() - tPos.x;
            double dz0  = mc.player.getZ() - tPos.z;
            double hLen = Math.sqrt(dx0 * dx0 + dz0 * dz0);
            double nx   = hLen > 1e-4 ? dx0 / hLen : 1.0;
            double nz   = hLen > 1e-4 ? dz0 / hLen : 0.0;
            rawX = tPos.x + nx * radius * 2.0;
            rawZ = tPos.z + nz * radius * 2.0;
            rawY = eyeY + 5.0;
        } else {
        ...
        if (escapeMode) {
            ...
            double closeness = MathHelper.clamp(
                    (float)(1.0 - dist / (radius * 3.0)), 0f, 1f);
            maxDownPitch = (float)(20.0 - closeness * 30.0);
        } else {
            maxDownPitch = 90f;
        }

        float tPitch = MathHelper.clamp(
                (float) -Math.toDegrees(Math.atan2(dy, hDist)), -90f, maxDownPitch);
    ...
    private boolean fireRocket() {
        ...
        if (swordRocketCycle.getValue() && autoSharpestSword.getValue()) {
            ...
            if (cycleStep % 2 == 0) {
                ...
                // NOTE: fireRocket metodu orijinal dosyada burada kesildi.
                // Devamını (PlayerInteractItemC2SPacket vb.) orijinal kaynaktan ekle.
            }
        }

        // TODO: buraya orijinal fireRocket metodunun geri kalanını ekle.
        return true;
    }
}
