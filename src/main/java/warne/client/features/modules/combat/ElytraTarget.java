...
public final class ElytraTarget extends Module {
    ...
    public void onPostSync(EventPostSync e) {
        ...
        // Aura'nın kendi "ElytraTarget" ayarı açıkken, o da aynı tick'te
        // hedefe göre yaw/pitch hesaplayıp paket gönderiyor. İki modül aynı
        // anda rotasyonu kontrol etmeye çalışınca birbirinin üstüne yazıyor,
        // bu da uçuşta takılma ve elytra'nın kendiliğinden kapanmasına
        // (yere inme) sebep oluyordu. Aura zaten bu işi yapıyorsa burada
        // devre dışı kalıyoruz.
        boolean auraÇakışıyor = ModuleManager.aura.isEnabled()
                && ModuleManager.aura.elytraTarget.getValue()
                && ModuleManager.aura.target != null;

        if (uçuyor) {
            if (hedefTakip.getValue() && hedefVar && !auraÇakışıyor) {
        ...
        boolean roketAtılacak = false;
        if (fişekBoost.getValue() && !(sadeceUçarken.getValue() && !uçuyor) && hedefVar) {
            double anHızÖnizleme = mc.player.getVelocity().length();
            boolean düşüyorÖnizleme = uçuyor && mc.player.getVelocity().y < -0.15;
            boolean hızUygun = anHızÖnizleme <= maksimumHız.getValue() || düşüyorÖnizleme;
            double hızOranıÖnizleme = düşüyorÖnizleme ? 0.0
                    : MathHelper.clamp(anHızÖnizleme / Math.max(maksimumHız.getValue(), 0.01), 0.0, 1.0);
            long efektifGecikmeÖnizleme = (long) (fişekGecikmesi.getValue() * (1.0 + hızOranıÖnizleme * 2.0));
            roketAtılacak = hızUygun && rocketTimer.passedMs(efektifGecikmeÖnizleme);
        }

        if (otoKılıç.getValue() && hedefVar && !roketAtılacak) {
        ...
        // Hız, üst sınıra (maksimumHız) yaklaştıkça roket atma gecikmesini
        // kademeli olarak uzatıyoruz (en fazla 3 kat). Böylece sabit bir
        // gecikme değeriyle "ya çok yavaş ya da limiti aşıp çok hızlı"
        // arasında sıkışmak yerine, modül hıza göre kendi kendine yumuşuyor.
        double hızOranı = düşüyor ? 0.0
                : MathHelper.clamp(anHız / Math.max(maksimumHız.getValue(), 0.01), 0.0, 1.0);
        long efektifGecikme = (long) (fişekGecikmesi.getValue() * (1.0 + hızOranı * 2.0));

        if (!rocketTimer.passedMs(efektifGecikme)) return;
