...
public final class ElytraTarget extends Module {
    ...
    public void onPostSync(EventPostSync e) {
        ...
        if (otoKılıç.getValue() && hedefVar && !uçuyor) {
    ...
    private void followAndOrbit(Entity target) {
        ...
        if (yawDelta || pitchDelta) {
            // Force onGround=false: this packet only fires while fall-flying,
            // and letting isOnGround() leak through here is what was causing
            // the server to think the player landed and cut the elytra flight.
            sendPacket(new PlayerMoveC2SPacket.LookAndOnGround(
                    finalYaw, finalPitch, false));
