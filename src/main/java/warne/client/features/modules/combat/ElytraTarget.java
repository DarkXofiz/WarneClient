...
public final class ElytraTarget extends Module {
    ...
    // Kılıç/crit sadece hedef gerçekten bu mesafenin içindeyken tetiklenir.
    // Menzil dışındayken saldırı denemesi göndermek anticheat'lere reach
    // (menzil hilesi) gibi görünüyordu — bu, "vurmaya çalıştığı için ban
    // atıyor" sorununun kaynağıydı.
    ...
    public void onPostSync(EventPostSync e) {
        ...
        // NOT: Önceden burada "GüvenliUçuş" adıyla mc.player.setVelocity() ile
        // çarpışma-önleme mantığı vardı. Client-side velocity manipülasyonu
        // sunucunun beklediği hareketle çelişip hız hilesi gibi algılanıyordu
        // (hareketi engelleme + ban) — bu yüzden tamamen kaldırıldı. Çarpışma
        // riskinden kaçınmak istiyorsan rotasyon/hedef seçimini ayarlaman
        // gerekiyor; bu modül artık oyuncunun fiziğine hiç müdahale etmiyor.

        if (uçuyor && hedefTakip.getValue() && hedefVar) {
            followTarget(hedef);
        } else {
            lastSentYaw   = Float.NaN;
            lastSentPitch = Float.NaN;
        ...
        handleKılıçVeFişek(hedef, hedefVar, hedefMenzilde, uçuyor);
    }
     ...
     */
    private void handleKılıçVeFişek(Entity hedef, boolean hedefVar, boolean hedefMenzilde, boolean uçuyor) {
        boolean fişekAktif = uçuyor && fişekBoost.getValue()
                && !(sadeceUçarken.getValue() && !uçuyor)
                && hedefVar;
        boolean kılıçAktif = otoKılıç.getValue() && hedefMenzilde;
