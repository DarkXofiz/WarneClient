...
public final class ElytraTarget extends Module {
    ...
    public void onPostSync(EventPostSync e) {
        ...
        boolean roketBuTick = false;

        if (!fişekBoost.getValue()) {
            // fişek kapalıysa hiçbir şey değişmiyor
        } else if (sadeceUçarken.getValue() && !uçuyor) {
            // uçmuyorken fişek atılmıyor
        } else if (!hedefVar) {
            // hedef yoksa fişek atılmıyor
        } else {
            ...
            if (anHız > maksimumHız.getValue() && !düşüyor) {
                rocketTimer.reset();
            } else if (rocketTimer.passedMs(fişekGecikmesi.getValue())) {
                roketBuTick = true;
            }
        }

        // Aynı tick içinde hem rotasyon hem item-kullanım (roket) paketi
        // göndermek anticheat'lere tipik bir bot/kombo imzası gibi görünüyor
        // ve bu "takılma"nın asıl kaynağıydı. Roket bu tick atılacaksa,
        // rotasyon güncellemesini bu tick atlıyoruz; bir sonraki tick'te
        // rotasyon devam ediyor. Bu iki eylem türünü ayrı tick'lere yayarak
        // daha insansı, ayrık bir paket deseni oluşturuyoruz.
        if (uçuyor) {
            if (hedefTakip.getValue() && hedefVar && !auraÇakışıyor && !roketBuTick) {
            ...
            if (otoCrit.getValue() && hedefVar && critTimer.passedMs(200) && !roketBuTick) {
                doCritSwing();
                critTimer.reset();
            }
        } else {
        ...
        if (otoKılıç.getValue() && hedefVar && !roketBuTick) {
