...
public final class ElytraTarget extends Module {
    ...
    public void onPostSync(EventPostSync e) {
        ...
        if (uçuyor) {
            ...
            /*
             * FIX: doCritPacket() elytra uçuşu sırasında sahte pozisyon
             * paketleri gönderiyordu (PositionAndOnGround). Sunucu elytra
             * trajectory'sini fizik motoruyla doğrular; bu sahte pozisyon
             * paketleri gerçek uçuş yörüngesiyle çelişince sunucu oyuncuyu
             * son geçerli pozisyona zorla geri çekiyordu (rubber-band).
             * Hedefe yaklaşınca rotasyon da agresifleştiği için bu çakışma
             * daha sık tetikleniyor, sonuç: "hareket edemiyorum ve ölüyorum".
             * Elytra uçuşunda kritik paketi artık hiç gönderilmiyor —
             * OtoCrit yalnızca YERDEYKEN (uçuş bittiğinde) çalışacak.
             */
        } else {
         ...
         */
        boolean elytraTakılı = mc.player.getEquippedStack(net.minecraft.entity.EquipmentSlot.CHEST)
                .isOf(net.minecraft.item.Items.ELYTRA);
    ...
    private void followAndOrbit(Entity target) {
        ...
        if (kaçışModu) {
            ...
            /*
             * FIX: eskiden rawY doğrudan eyeY + 5.0 idi — kaçışModu'na
             * girdiğimiz an mevcut yükseklikten bağımsız sabit bir hedef
             * veriliyordu. Oyuncu o an hedefin altındaysa hedefPitch aşağı
             * yönlü çıkıyor, sonra pitchMaxAşağı=-15f ile aniden yukarı
             * zorlanıyordu. Bu çakışma smoothing gecikmesiyle birleşince
             * ilk temas anında gözle görülür bir "aşağı sarkma" yaratıyordu.
             * Artık hedef Y, oyuncunun MEVCUT yüksekliğine göre veriliyor —
             * ani sıçrama yok, sarkma da oluşmuyor.
             */
            rawY = mc.player.getY() + mc.player.getEyeHeight(mc.player.getPose()) + 3.0;
        } else {
        ...
        if (kaçışModu) {
            /*
             * FIX: -15f çok sert bir üst sınırdı (neredeyse hep yukarı
             * bakmaya zorluyordu), rawY artık kademeli yükseldiği için
             * bu sınırı gevşetmek yeterli — ani zıplama davranışı kalktı.
             */
            pitchMaxAşağı = 10f;
