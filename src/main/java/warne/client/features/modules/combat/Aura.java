...
public class Aura extends Module {
    ...
    public boolean isInRange(Entity entity) {
        // Zaten kilitli olan hedefe küçük bir tolerans payı (+1.5 blok)
        // tanınıyor. Vurunca oluşan knockback, hedefi menzilin az dışına
        // itebiliyordu; sıkı sınır bunu anında "menzil dışı" sayıp hedefi
        // bırakıyordu ("bir kere vurup bırakıyor" sorunu buradan geliyordu).
        // Yeni bir hedef seçilirken bu tolerans uygulanmıyor — sadece halihazırda
        // kilitli olan hedef için geçerli, böylece ilk hedef seçimi sıkı kalıyor.
        boolean zatenKilitli = entity == target;
        double efektifMenzil = Math.sqrt(getSquaredRotateDistance() + 4) + (zatenKilitli ? 1.5 : 0.0);

        if (PlayerUtility.squaredDistanceFromEyes(entity.getPos().add(0, entity.getEyeHeight(entity.getPose()), 0))
                > efektifMenzil * efektifMenzil) {
            return false;
        }

        float[] rotation;
        float halfBox = (float) (entity.getBoundingBox().getLengthX() / 2f);

        // уменьшил частоту выборки
        for (float x1 = -halfBox; x1 <= halfBox; x1 += 0.15f) {
            for (float z1 = -halfBox; z1 <= halfBox; z1 += 0.15f) {
                for (float y1 = 0.05f; y1 <= entity.getBoundingBox().getLengthY(); y1 += 0.25f) {
                    if (PlayerUtility.squaredDistanceFromEyes(new Vec3d(entity.getX() + x1, entity.getY() + y1, entity.getZ() + z1)) > getSquaredRotateDistance())
                        continue;

                    rotation = Managers.PLAYER.calcAngle(new Vec3d(entity.getX() + x1, entity.getY() + y1, entity.getZ() + z1));
