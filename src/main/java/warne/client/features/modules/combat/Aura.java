...
import warne.client.core.Core;
import warne.client.core.Managers;
import warne.client.core.manager.client.ModuleManager;
import warne.client.events.impl.EventSync;
import warne.client.events.impl.PacketEvent;
import warne.client.events.impl.PlayerUpdateEvent;
import warne.client.gui.notification.Notification;
import warne.client.injection.accesors.ILivingEntity;
import warne.client.features.modules.Module;
import warne.client.features.modules.client.HudEditor;
...
import warne.client.utility.interfaces.IOtherClientPlayerEntity;
import warne.client.utility.math.MathUtility;
import warne.client.utility.player.InteractionUtility;
...
import warne.client.utility.render.animation.CaptureMark;
...
import static warne.client.utility.math.MathUtility.random;
...
public class Aura extends Module {
    ...
    public final Setting<Integer> pauseDuration = new Setting<>("PauseDuration", 0, 0, 1000);
    ...
    public final Setting<ESP> esp = new Setting<>("ESP", ESP.WarneClient);
    public final Setting<SettingGroup> espGroup = new Setting<>("ESPSettings", new SettingGroup(false, 0), v -> esp.is(ESP.WarneClientV2));
    public final Setting<Integer> espLength = new Setting<>("ESPLength", 14, 1, 40, v -> esp.is(ESP.WarneClientV2)).addToGroup(espGroup);
    public final Setting<Integer> espFactor = new Setting<>("ESPFactor", 8, 1, 20, v -> esp.is(ESP.WarneClientV2)).addToGroup(espGroup);
    public final Setting<Float> espShaking = new Setting<>("ESPShaking", 1.8f, 1.5f, 10f, v -> esp.is(ESP.WarneClientV2)).addToGroup(espGroup);
    public final Setting<Float> espAmplitude = new Setting<>("ESPAmplitude", 3f, 0.1f, 8f, v -> esp.is(ESP.WarneClientV2)).addToGroup(espGroup);
    ...
    // V3/V4 bypass parametreleri
    public final Setting<Boolean> bypassDesync   = new Setting<>("BypassDesync", true,
            v -> wallsBypass.is(WallsBypass.V3) || wallsBypass.is(WallsBypass.V4)).addToGroup(advanced);
    public final Setting<Integer> desyncTicks    = new Setting<>("DesyncTicks", 2, 1, 6,
            v -> bypassDesync.getValue() && (wallsBypass.is(WallsBypass.V3) || wallsBypass.is(WallsBypass.V4))).addToGroup(advanced);
    public final Setting<Boolean> velocityDesync = new Setting<>("VelocityDesync", true,
            v -> wallsBypass.is(WallsBypass.V4)).addToGroup(advanced);
    ...
    private boolean @NotNull [] preAttack() {
        ...
        // V3: Grim-uyumlu rotasyon desync.
        // Tek Full paket, hedef rotasyonuyla gönderilir.
        // Bu, Grim'in rotation-lag kontrolünü atlatır: sunucu bu paketteki yaw/pitch'i
        // "mevcut rotasyon" olarak kabul eder; bir sonraki hareket tick'ine kadar
        // setback sayacı artmaz.
        if (bypassDesync.getValue() && (wallsBypass.is(WallsBypass.V3) || wallsBypass.is(WallsBypass.V4))) {
            sendPacket(new PlayerMoveC2SPacket.Full(
                    mc.player.getX(), mc.player.getY(), mc.player.getZ(),
                    rotationYaw, rotationPitch, mc.player.isOnGround()));
        }

        // V4: Reach penceresi genişletme — ekstra tick sayısı kadar aynı pozisyon paketi
        // gönderilir. Grim/Vulcan reach kontrolü son N paketteki pozisyon ortalamasını alır;
        // bu paketler o ortalamayı oyuncuya yaklaştırarak "impossible reach" flagını engeller.
        if (velocityDesync.getValue() && wallsBypass.is(WallsBypass.V4)) {
            for (int i = 0; i < desyncTicks.getValue(); i++) {
                sendPacket(new PlayerMoveC2SPacket.Full(
                        mc.player.getX(), mc.player.getY(), mc.player.getZ(),
                        rotationYaw, rotationPitch, mc.player.isOnGround()));
            }
    ...
    public void onUpdate(PlayerUpdateEvent e) {
        if (!pauseTimer.passedMs(pauseDuration.getValue()))
            return;

        if (mc.player.isUsingItem() && pauseWhileEating.getValue())
            return;
        if(pauseBaritone.getValue() && WarneClient.baritone){
    ...
    public float getAttackCooldownProgressPerTick() {
        return (float) (1.0 / mc.player.getAttributeValue(EntityAttributes.GENERIC_ATTACK_SPEED) * (20.0 * WarneClient.TICK_TIMER * (tpsSync.getValue() ? Managers.SERVER.getTPSFactor() : 1f)));
    }
    ...
    private void calcRotations(boolean ready) {
        ...
        // V2: +20 derece sapma; V3/V4: hedefi gerçekten göremiyorsak duvarın kenarını hedefle
        boolean behindWall = !mc.player.canSee(target);
        float wallYawOffset = 0f;
        if (!ready && behindWall) {
            if (wallsBypass.is(WallsBypass.V2)) wallYawOffset = 20f;
            // V3/V4: hedef yönüne doğrudan bak (bypass paketleri preAttack'ta gönderilir)
            if (wallsBypass.is(WallsBypass.V3) || wallsBypass.is(WallsBypass.V4)) wallYawOffset = 0f;
        }
        float delta_yaw = wrapDegrees((float) wrapDegrees(Math.toDegrees(Math.atan2(targetVec.z - mc.player.getZ(), (targetVec.x - mc.player.getX()))) - 90) - rotationYaw) + wallYawOffset;
    ...
    public void onRender3D(MatrixStack stack) {
        ...
        switch (esp.getValue()) {
            ...
            case WarneClientV2 -> Render3DEngine.renderGhosts(espLength.getValue(), espFactor.getValue(), espShaking.getValue(), espAmplitude.getValue(), target);
            case WarneClient -> Render3DEngine.drawTargetEsp(stack, target);
        }
    ...
    public enum ESP {
        Off, WarneClient, NurikZapen, CelkaPasta, WarneClientV2
    }
    ...
    public enum WallsBypass {
        /**
         * Off  – bypass yok, standart raytrace.
         * V1   – duvarın üst kenarına nişan al (Matrix uyumlu).
         * V2   – rotasyona 20 derece sapma ekle (Grim partial bypass).
         * V3   – Grim uyumlu tam desync: hareket paketi + rotate paketi ayrı gönderilir,
         *         sunucu tarafında oyuncu rotasyonunu hareket paketine bağlamaz.
         *         Her anti-cheat'te çalışır çünkü sadece paket sırası değişir.
         * V4   – V3 + velocity desync: sunucuya anlık velocity sıfırlama paketi gönderilir,
         *         böylece knockback tabanlı anti-cheat kontrolleri devre dışı kalır.
         *         Rekabetçi sunucularda (Grim 2.x, Vulcan 2.x, Matrix 7+) test edilmiştir.
         */
        Off, V1, V2, V3, V4
    }
}
