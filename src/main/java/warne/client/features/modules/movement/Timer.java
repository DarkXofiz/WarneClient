...
import warne.client.events.impl.PostPlayerUpdateEvent;
...
public class Timer extends Module {
    ...
    public void onUpdate() {
        switch (mode.getValue()) {
            case Normal -> WarneClient.TICK_TIMER = speed.getValue();
            case Matrix -> {
                ...
                WarneClient.TICK_TIMER = Math.max(speed.getValue(), 1f);
            ...
            case Grim -> {
                if (energy <= 0 || !isKeyPressed(boostKey) || WarneClient.core.getSetBackTime() < 2000) {
                    WarneClient.TICK_TIMER = 1f;
                    return;
                }
    ...
    public void onPacketReceive(PacketEvent.Receive e) {
        if (mode.is(Mode.Grim)) {
            if (e.getPacket() instanceof CommonPingS2CPacket
                    && WarneClient.core.getSetBackTime() > 2000) {
