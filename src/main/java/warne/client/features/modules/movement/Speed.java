...
import warne.client.injection.accesors.IInteractionManager;
import warne.client.features.modules.Module;
import warne.client.setting.Setting;
import warne.client.utility.interfaces.IEntity;
...
import static warne.client.utility.player.MovementUtility.isMoving;
...
public class Speed extends Module {
    ...
    private warne.client.utility.Timer elytraDelay = new warne.client.utility.Timer();
    private warne.client.utility.Timer startDelay = new warne.client.utility.Timer();
    ...
    public void onSync(EventSync e) {
        ...
        if (mode.getValue() == Mode.MatrixJB) {
            ...
            if (MovementUtility.isMoving() && closeToGround && mc.player.fallDistance <= 0) {
                ...
                WarneClient.TICK_TIMER = matrixJBSpeed.getValue();
    ...
    public void modifyVelocity(EventPlayerTravel e) {
        if (mode.getValue() == Mode.GrimEntity && !e.isPre() && WarneClient.core.getSetBackTime() > 1000) {
        ...
        if ((mode.is(Mode.GrimEntity2) || mode.is(Mode.GrimCombo)) && !e.isPre() && WarneClient.core.getSetBackTime() > 1000 && MovementUtility.isMoving()) {
            int collisions = 0;
    ...
    public void onMove(EventMove event) {
        ...
        if (MovementUtility.isMoving()) {
            WarneClient.TICK_TIMER = useTimer.getValue() ? 1.088f : 1f;
