package warne.client.gui.clickui;
...
import warne.client.features.modules.client.BaritoneSettings;
import warne.client.features.modules.client.ClickGui;
import warne.client.features.modules.client.HudEditor;
import warne.client.gui.clickui.impl.SearchBar;
...
public class Category extends AbstractCategory {
    ...
    public Category(Module.Category category, ArrayList<Module> features, float x, float y, float width, float height) {
        ...
        ICON = Identifier.of("warneclient", "textures/gui/headers/" + (Module.Category.isCustomCategory(category) ? "stock" : category.getName().toLowerCase()) + ".png");

        if (category.getName().equals("Client"))
            buttons.add(new SearchBar());

        features.forEach(feature -> {
            if (!(feature instanceof BaritoneSettings) || WarneClient.baritone)
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        if (FontRenderers.categories != null) {
