...
import warne.client.core.manager.client.MacroManager;
...
import warne.client.gui.thundergui.ThunderGui;
...
import static warne.client.features.modules.Module.fullNullCheck;
...
public final class Core {
    ...
    public void onTick(PlayerUpdateEvent event) {
        ...
        for (PlayerEntity p : mc.world.getPlayers()) {
            if (p.isDead() || p.getHealth() == 0)
                WarneClient.EVENT_BUS.post(new EventDeath(p));
        }
    ...
    public void drawGps(DrawContext e) {
        if (WarneClient.gps_position != null) {
            float dst = getDistance(WarneClient.gps_position);
            float xOffset = mc.getWindow().getScaledWidth() / 2f;
            float yOffset = mc.getWindow().getScaledHeight() / 2f;
            float yaw = getRotations(new Vec2f(WarneClient.gps_position.getX(), WarneClient.gps_position.getZ())) - mc.player.getYaw();
